...
def pl_test012(test_suite_details):
    ...
    try:
        '''
        TODO:
        need to rethink how to make multiple test connects happen with different test names?
        Using sys._getframe().f_code.co_name works in a single connect per test paradigm.
        '''
        test_driver.testname = test_driver.testname + "_01"
        VistA1 = test_driver.connect_VistA(test_suite_details)
        pl = PLActions(VistA1, user='fakedoc1', code='1Doc!@#$')
...
def startmon(test_suite_details):
    ...
    try:
        VistA1 = test_driver.connect_VistA(test_suite_details)
        VistA1.startCoverage(routines=['GMPL*']) #TODO: move out of OSEHRA helper?
        '''
        Close Vista
        '''
        VistA1.write('^\r^\r^\r')
...
def stopmon (test_suite_details):
    ...
    try:
        # Connect to VistA
        VistA1 = test_driver.connect_VistA(test_suite_details)
        VistA1.stopCoverage(path=(test_suite_details.result_dir + '/' + 'ProblemList_coverage.txt')) #TODO: move out of OSEHRA helper?
        '''
        Close Vista
        '''
        VistA1.write('^\r^\r^\r')
